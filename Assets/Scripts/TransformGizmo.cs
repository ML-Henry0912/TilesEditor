...
//          ⚠️ 目前旋轉採用 FromToRotation + Dot 判斷，若滑鼠拖曳跨越圓環180度或 Gizmo 有鏡像/縮放時，
//          可能出現旋轉跳變或方向反轉的風險。
//          所有 Gizmo 共用同一份靜態材質，顏色與透明度（80%）以 MaterialPropertyBlock 設定，避免記憶體浪費。
//          PlaneGizmo 會自動產生正反兩面，確保雙面皆可顯示顏色與高光，反面僅作顯示不參與互動。
//          所有互動偵測（軸、平面、旋轉）皆以數學計算為主，不依賴 Collider，確保精確度與效能。
//          所有 handle 皆為 TransformGizmo 的子物件，請勿手動移除或更改父子結構，否則可能導致互動異常。
// =============================================
using System;
using UnityEngine;
